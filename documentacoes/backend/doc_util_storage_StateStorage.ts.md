# Documentação: StateStorage.ts

## Nome do Arquivo
`src/backend/util/storage/StateStorage.ts`

## Propósito
Arquivo de definições de interfaces TypeScript que estabelece contratos para gerenciamento de estados de chat ativos, fornecendo tipagem forte e isolamento de responsabilidades no sistema de armazenamento. Versão original das interfaces.

## Funcionamento
Este arquivo contém apenas definições de interfaces TypeScript que especificam contratos para implementações de armazenamento de estados. Não contém lógica executável, apenas declarações de tipos para garantir consistência e segurança de tipos em todo o sistema. Esta é a versão original, enquanto StateStorage-fixed.ts contém uma versão corrigida.
...
import { StateStorage, ActiveChatState } from './StateStorage';
...
- **Versão Original**: Esta é a versão original das interfaces (há também StateStorage-fixed.ts)
