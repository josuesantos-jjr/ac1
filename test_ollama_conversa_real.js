// Teste avançado para validar o Ollama com histórico real de conversa
const fs = require('fs');
const path = require('path');

console.log('🧪 Testando Ollama com histórico real de conversa...');
...
async function testOllamaWithRealHistory() {
  try {
    ...
    // 4. Testa a chamada ao Ollama
    console.log('\n🚀 Testando chamada ao Ollama...');
    
    // Importa o serviço Ollama
    const { mainOllamaService } = require('./src/backend/service/ollamaService.ts');
    
    const startTime = Date.now();
    
    const response = await mainOllamaService({
      currentMessage: newMessage,
      chatId: '5519989808901@c.us',
      clearHistory: false, // Mantém o histórico para continuidade da conversa
      maxRetries: 3,
      __dirname: './clientes/CMW',
      history: convertedHistory
    });
    
    const duration = Date.now() - startTime;
    
    console.log(`\n✅ Ollama respondeu em ${duration}ms:`);
    ...
    const validations = {
      'Resposta não vazia': response && response.trim().length > 0,
      'Em português': isPortuguese(response),
      'Contextual': isContextualResponse(response, newMessage, chatHistory),
      'Formato adequado': !response.includes('ERROR') && !response.includes('FALHA')
    };
    ...
    const secondResponse = await mainOllamaService({
      currentMessage: secondMessage,
      chatId: '5519989808901@c.us',
      clearHistory: false,
      maxRetries: 3,
      __dirname: './clientes/CMW',
      history: [...convertedHistory, { role: 'user', content: newMessage }, { role: 'assistant', content: response }]
    });
    
    console.log(`\n✅ Segunda resposta (${Date.now() - startTime}ms):`);
...
function generateTestReport(reportData) {
  ...
  console.log(`   Tempo: ${reportData.duration}ms`);
