...
export default function FollowUpConfigModal({
  ...
  const fetchConfig = useCallback(async () => {
    ...
    console.log(
      `[FollowUpConfigModal JS] Fetching config for: clientName=${clientId}`
    );
    setIsLoading(true);
    setError(null);
    try {
      ...
      // Garante que todos os campos esperados existam e ajusta arrays
      const mergedConfig = { ...defaultConfig, ...data.config };
      mergedConfig.niveis = parseInt(mergedConfig.niveis, 10) || 5; // Garante que níveis seja número
      mergedConfig.promptsPorNivel = mergedConfig.promptsPorNivel || [];
      mergedConfig.intervalosDias = mergedConfig.intervalosDias || [];

      mergedConfig.promptsPorNivel = mergedConfig.promptsPorNivel.slice(
        0,
        mergedConfig.niveis
      );
      ...
      mergedConfig.intervalosDias = mergedConfig.intervalosDias.slice(
        0,
        mergedConfig.niveis
      );
          ...
          <div className="form-content">
              ...
              Array.from({ length: config.niveis || 0 }).map((_, index) => (
                <div key={`prompt-${index}`} className="form-group">
                  ...
                  <textarea
                    ...
                    rows="2"
                    disabled={isLoading}
                    placeholder={`Prompt para o nível ${index + 1}. Use {conversationHistory} para histórico.`}
                  />
                  <p className="input-description">
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                rows="3"
                disabled={isLoading}
                placeholder="Prompt para a IA decidir se inicia o follow-up. Use {conversationHistory} para histórico. Resposta esperada: SIM ou NAO."
              />
              <p className="input-description">
                Prompt para a IA decidir se inicia o follow-up. Use{' '}
                {'{conversationHistory}'} para histórico. Resposta esperada: SIM
                ou NAO.
              </p>
            </div>
