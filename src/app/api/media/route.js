...
import * as fs from 'node:fs/promises'; // Importa fs/promises como 'fs'
...
export async function POST(request) {
  try {
    ...
    await fs.mkdir(clientMediaDir, { recursive: true });
    ...
    try {
      await syncManager.saveClientData(clientId, {
        media: { [fileName]: mediaInfo }
      });
      console.log(`[API /api/media POST] Mídia salva no SQLite para ${clientId}`);
    } catch (sqliteError) {
      console.error(`[API /api/media POST] Erro ao salvar no SQLite:`, sqliteError);
