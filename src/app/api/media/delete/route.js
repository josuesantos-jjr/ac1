    ...
    if (updated) {
      ...
      try {
        await syncManager.saveClientData(clientId, {
          listas: { [listaNome]: listaData }
        });
        console.log(`[API Media Delete] Lista atualizada no SQLite para ${clientId}`);
      } catch (sqliteError) {
        console.error(`[API Media Delete] Erro ao salvar no SQLite:`, sqliteError);
