...
async function lerListasDoCliente(listasDirPath, clientId) {
  ...
  try {
    ...
    const listasPromises = jsonFiles.map(async (file) => {
      ...
      try {
        ...
        return {
         ...
         console.error(
           `[API] Erro ao ler ou parsear o arquivo ${filePath} para ${clientId}:`,
           readError
         );
         console.error(`[API] Detalhes do erro:`, readError.message, readError.stack);
    ...
    if (err.code === 'ENOENT') {
      ...
      console.log(`Cliente ${clientId} pode precisar ser configurado corretamente.`);
      listas = [];
    } else {
      // Para outros erros de leitura do diretório, loga e relança para o handler principal
      console.error(
        `[API] Erro inesperado ao ler diretório de listas ${listasDirPath} para ${clientId}:`,
        err
      );
      console.error(`[API] Detalhes do erro:`, err.message, err.stack);
      throw new Error(
...
export async function GET(request) {
  console.log('[API] Chamada para /api/disparo-status iniciada.');
  const { searchParams } = new URL(request.url);
  const clientId = searchParams.get('clientId');

  if (!clientId) {
    return NextResponse.json(
      { error: 'Parâmetro "clientId" ausente' },
      { status: 400 }
    );
  }

  const clienteNome = clientId;
  ...
  try {
    ...
    console.log(`[API] Caminho base: ${basePath}`);
    ...
    console.log(`[API] Caminho para o diretório de listas: ${listasDirPath}`);

    // 2. Ler e processar os arquivos JSON das listas
    const listas = await lerListasDoCliente(listasDirPath, clientId);
    console.log(`[API] Listas lidas para ${clientId}:`, listas);
    ...
    return NextResponse.json(statusCalculado, { status: 200 });
 
  } catch (error) {
    console.error(`[API] Erro geral ao buscar status de disparo para ${clientId}:`, error);
    console.error(`[API] Detalhes do erro:`, error.message, error.stack);
