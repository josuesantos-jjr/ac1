...
async function ensureListasDir(clientId) {
  const listasPath = path.join(process.cwd(), 'clientes', clientId, 'config', 'listas');
  ...
  try {
    ...
    try {
      await syncManager.saveClientData(clientId, {
        listas: { [nomeLista]: listaCompletaData }
      });
      console.log(`[API /api/listas/upload] Lista "${nomeLista}" salva no SQLite para ${clientId}`);
    } catch (sqliteError) {
      console.error(`[API /api/listas/upload] Erro ao salvar no SQLite:`, sqliteError);
