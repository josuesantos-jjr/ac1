  ...
  try {
    ...
    try {
      await syncManager.saveClientData(clientId, {
        followupConfig: configData
      });
      console.log(`[FollowUp Config] Configuração salva no SQLite para ${clientId}`);
    } catch (sqliteError) {
      console.error(`[FollowUp Config] Erro ao salvar no SQLite:`, sqliteError);
