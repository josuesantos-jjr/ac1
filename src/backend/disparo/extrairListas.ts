  ...
  try {
    ...
    for (const lista of listas) {
      try {
        ...
        // 🔄 SALVAR NO SQLITE (sincronização automática) - lista extraída
        try {
          await syncManager.saveClientData(clientePath, {
            extractedLists: { [nomeLista]: dadosTabela }
          });
          console.log(`[extrairListas] Lista ${nomeLista} salva no SQLite para ${clientePath}`);
        } catch (sqliteError) {
          console.error(`[extrairListas] Erro ao salvar no SQLite:`, sqliteError);
