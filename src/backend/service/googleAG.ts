...
export const mainGoogleAG = async ({
  ...
  while (retries < maxRetries) {
    try {
      ...
      // Verifica se é erro de conectividade ou API
      ...
      if (isNetworkError) {
        console.log(`Erro de conectividade detectado: ${errorMessage}`);
      }

      retries++;
      console.log(`Tentativa ${retries} de ${maxRetries}...`);

      // Se é erro de autenticação, não tenta novamente
      if (errorMessage.includes('Invalid API key') || errorMessage.includes('unauthorized') || errorMessage.includes('403')) {
        console.error('Erro de autenticação - chave da API inválida');
        return 'Erro de autenticação na API Gemini. Verifique a chave da API.';
      }

      // Backoff exponencial para diferentes tipos de erro
      const baseDelay = isNetworkError ? 3000 : 2000;
      const delay = Math.min(baseDelay * Math.pow(1.5, retries), 15000);
